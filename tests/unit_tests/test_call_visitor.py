...
def test_range_call():
    ...
    rtype, struct = visit_call(node, Context(predicted_struct=Unknown()))
...
def test_dict_call():
    ...
    rtype, struct = visit_call(
        call_node, Context(predicted_struct=Unknown.from_node(call_node)))
...
def test_str_method_calls():
    ...
    call_node = parse(template).find(nodes.Call)
    ctx = Context(return_struct_cls=Unknown, predicted_struct=Unknown.from_node(call_node))
